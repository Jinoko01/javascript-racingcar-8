...
import CarFactory from './entities/CarFactory.js';
import RacingController from './entities/RacingController.js';

class App {
  async run() {
    const inputProcessor = new InputProcessor();
    const { carNames: carNamesInput, roundCount: roundCountInput } = await inputProcessor.input();

    const parser = new Parser();
    const { carNames, roundCount } = parser.parse(carNamesInput, roundCountInput);
